...
# rebuild.sh for noir-lang/noir
# Runs on an already-checked-out source tree (files at current directory, not source-repo/).
...
# ENV=dev so docusaurus uses 'docs/' path instead of 'processed-docs/'
echo "[INFO] Running docusaurus build..."
ENV=dev ./node_modules/.bin/docusaurus build

# Verify build output
if [ -d "build" ]; then
    FILE_COUNT=$(find build -type f | wc -l)
    echo "[INFO] Build success! Generated $FILE_COUNT files in build/"
    ls build/ | head -20
else
    echo "[ERROR] build/ directory not found after docusaurus build"
    exit 1
fi
